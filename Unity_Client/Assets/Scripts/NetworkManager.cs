...
public class NetworkManager : MonoBehaviour
{
    ...
    // AGA B�Y� BURADA: Fonksiyon parametreleri eskisi gibi temiz (sadece 4 tane)!
    ...
    private IEnumerator PostRequest(string message, string npcId, string action, string voiceModel)
    {
        ...
        // VER�Y� ELDEN TA�IMIYORUZ, D�REKT MERKEZDEN �EK�YORUZ!
    ...
    // SES FONKS�YONU: Parametreleri yine temiz tuttuk!
    ...
    private IEnumerator PostVoiceRequest(byte[] voiceData, string npcId, string action, string voiceModel)
    {
        ...
        // MERKEZDEN G�ZL�CE �EK�P FORMA EKL�YORUZ!
        if (GameManager.Instance != null)
        {
        ...
        if (request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError)
        {
            Debug.LogError("Ses G�nderme Hatas�: " + request.error);
            if (DialogSystem.Instance != null)
                DialogSystem.Instance.ReceiveResponse("Seni duyamad�m, ba�lant� koptu.", "", null);
        }
        else
        {
            string jsonResponse = request.downloadHandler.text;
            Debug.Log("<color=cyan>SES CEVABI GELD�: </color>" + jsonResponse);
    ...
    private IEnumerator PostEndChat(string playerId)
    {
        ...
        {
            Debug.Log("<color=orange>[*] Sohbet bitti sinyali Python'a ula�t�. Haf�za �zetleniyor.</color>");
        }
        else
        {
            Debug.LogError("End Chat sinyali g�nderilemedi: " + request.error);
        }
    }
    ...
    private IEnumerator DownloadAndPlayAudio(string url, string message, string emotion)
    {
        ...
        using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip(finalUrl, AudioType.WAV))
        {
            ...
            {
                Debug.LogError("Ses indirme hatas�: " + www.error);
    ...
    // AGA BÜYÜ BURADA: Unity kapanırken Coroutine'ler anında durur. 
    // Bu yüzden "yield return" KULLANMADAN direkt paketi ateşleyip bırakıyoruz.
    // AGA BÜYÜ BURADA: Asenkron yerine SENKRON istek atıyoruz. Oyun kapanmadan önce zorla bekler.
    public void SendShutdownSignal()
    {
        try
        {
            ...
            // Yanıtı bekle ki paketin PC'den çıktığına %100 emin olalım
